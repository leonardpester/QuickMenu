...
$(document).ready(function() {
  ...
  groupsTable = $('.dataTable').on('draw.dt', function ( e, settings, json, xhr ) {
    ...
    'order': [[ 0, 'desc' ]],
    'ajax': {
      'url': baseURL + 'numbers/getDataTableRowsGroups',
      'type': 'POST',
      'data': function(data){
    ...
    },
    {
      'render': function (data, type, row) {
        return (data == 'active' ? '<i class="fa fa-circle text-success"></i> ' + __('ON') : '<i class="fa fa-circle text-danger"></i> ' +  __('OFF'));
      },
      'targets': 4
    }]
  });
  ...
  // createSelect2Instance('*[name="callerid_group"]', {
  //   placeholder: 'Caller ID group',
  //   ajax: true,
  //   url: baseURL+'numbers/getCallerIdGroups/index'
  // });
...
//   bootbox.confirm(__('Are you sure you want to delete this group?') + '<br /><i><small>' + __('This action cannot be undone!') + '</small></i>', function(result) {
//     if(result) {
//       hideLoader($(table_selector));
//       $.ajax({
//         url: baseURL+'numbers/deleteGroup/'+group_id,
//         type: 'post',
//         data: { token: Cookies.get('token') }
